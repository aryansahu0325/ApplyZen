...
let html = fs.readFileSync('e:/ApplyZen/scratch/app_status.html', 'utf8');
...
// Map pseudo-classes that might have escaped the boundary checks (like `/30` etc)
content = content.replace(/border-slate-300\/30/g, 'border-slate-300/30');
...
content = content.replace(/style="([^"]+)"/g, (match, p1) => {
    ...
    parts.forEach(part => {
        ...
        if (k && v) {
            ...
            styleObj[camelK] = v.replace(/'/g, '"'); // e.g. 'FILL' 1 -> "FILL" 1 or something, wait React needs standard strings.
            // React handles: { fontVariationSettings: "'FILL' 1" }
        }
    });
...
fs.writeFileSync('e:/ApplyZen/src/pages/ApplicationStatus.jsx', reactComponent);
console.log("Successfully generated ApplicationStatus.jsx");
